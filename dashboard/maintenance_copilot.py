...
if user_query:
    ...
    if "red" in user_query.lower():
        st.info("Motor A shows a 20% vibration increase (Pattern: Bearing Wear). RUL is predicted at 12 cycles.")
    elif "failing" in user_query.lower():
        st.info("RUL is <10 cycles. German logs show a 'Lagerdefekt' (bearing defect) was noted in München 2 weeks ago, and vibration has since spiked by 25%.")
    else:
        st.info("[LLM Response Placeholder] Contextual insights will appear here based on OEE, RUL, and ERP logs.")
else:
    st.caption("Type a question to get executive or root-cause insights.")

st.caption("Data source: DuckDB gold marts. LLM integration is a placeholder for future enhancement.")
