...
def model(dbt, session):
    ...
    session.execute(
        "CREATE TABLE rf_feature_importance (feature VARCHAR, importance DOUBLE)"
    )
    for _, row in rf_feature_importance.iterrows():
        session.execute(
            "INSERT INTO rf_feature_importance VALUES (?, ?)",
    ...
    for _, row in vif_data.iterrows():
        session.execute(
            "INSERT INTO lr_vif VALUES (?, ?)", (row["feature"], float(row["VIF"]))
        )
    ...
    session.execute(
        "INSERT INTO lr_diagnostics VALUES (?, ?)", ("durbin_watson", float(dw_stat))
    )

    # Store all RMSEs for model selection
    results.append(
        {"model_name": "LinearRegression", "rmse": lr_rmse, "variant": "vanilla"}
    )
    ...
    results.append(
        {"model_name": "ElasticNet", "rmse": enet_rmse, "variant": "elasticnet"}
    )
    ...
    # Fit XGBoost with early stopping only if not in cross-validation
    # RandomizedSearchCV will clone the estimator, so pass early_stopping_rounds and eval_set via fit_params
    # Only pass early_stopping_rounds and eval_set if not in cross-validation
    xgb_search.fit(
        X_train,
        y_train
    )
    # After search, refit best estimator with early stopping
    xgbr = xgb_search.best_estimator_
    # Only pass early_stopping_rounds if supported
    if hasattr(xgbr, "fit") and "early_stopping_rounds" in xgbr.fit.__code__.co_varnames:
        xgbr.fit(
            X_train,
            y_train,
    ...
    for train_idx, val_idx in kf.split(X_train):
        ...
        xgbr_cv.fit(X_train.iloc[train_idx], y_train.iloc[train_idx])
        preds_cv = xgbr_cv.predict(X_train.iloc[val_idx])
        try:
    ...
    explainer = shap.Explainer(xgbr.predict, X_train)
    ...
    session.execute(
        "CREATE TABLE xgb_feature_importance (feature VARCHAR, mean_abs_shap DOUBLE)"
    )
    for _, row in shap_importance.iterrows():
        session.execute(
            "INSERT INTO xgb_feature_importance VALUES (?, ?)",
    ...
    session.execute(
        "CREATE TABLE model_performance (model_name VARCHAR, rmse DOUBLE, is_best BOOLEAN, timestamp TIMESTAMP)"
    )
    ...
    col_defs = ", ".join(
        [f"{k} DOUBLE" for k in cluster_profiles[0] if k != "cluster"]
        + ["cluster INTEGER"]
    )
    ...
    for profile in cluster_profiles:
        ...
        session.execute(
            f"INSERT INTO kmeans_cluster_diagnostics VALUES ({placeholders})",
            tuple(vals),
        )
